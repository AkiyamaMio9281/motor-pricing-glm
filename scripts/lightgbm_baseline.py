...
def render(r: dict) -> list[str]:
    ...
    out = [
        ...
        *md_table(
        ...
        "",
        f"The variance power is {bm.VARIANCE_POWER}. A compound Poisson sum of Gamma claims with shape α is Tweedie with",
        f"power (α + 2) / (α + 1). The capped claims' shape in `docs/model-diagnostics.md` is {shape}, which gives",
